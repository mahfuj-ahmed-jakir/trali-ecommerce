...
const SubCategoryApprove = () => {
  let [brand, setBrand] = useState([]);

  useEffect(() => {
    async function brnd() {
      let { data } = await axios.get("http://localhost:8000/disapprove-sub-category");
      setBrand(data);
    }
    brnd();
  }, []);
  ...
  let handleApprove = (id) => {
    async function brnd() {
      let { data } = await axios.put(`http://localhost:8000/admin-approve-sub-category/${id}`);
  ...
  let handleDelete = (id) => {
    async function brnd() {
      let { data } = await axios.delete(`http://localhost:8000/admin-delete-sub-category/${id}`);
      const notify = () => toast(data);
      notify();
    }
    brnd();
  ...
  return (
    <div>
      ...
      {brand[0] ? (
        ...
        <h5 className="brand_warning">You have don't Sub Category approval!</h5>
      )}
    </div>
  );
};

export default SubCategoryApprove;
